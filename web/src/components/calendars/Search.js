...
const Search = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            ...
            const eventsArray = Object.entries(
                events || {}
            ).map(([id, data]) => ({ id, ...data }));
            const calendarRef = firebase.database.ref(
            ...
            if (fetchCalendar && fetchCalendar.eventsCalendar) {
                const filteredEventsArray = eventsArray.filter(
                    (event) => !fetchCalendar.eventsCalendar[event.id]
                );
                ...
                {filteredEvents.map((event) => (
                    ...
                    >
                        ...
                        <Button
                            ...
                            color={
                                addedEvents[event.id] ? "default" : "primary"
                            }
