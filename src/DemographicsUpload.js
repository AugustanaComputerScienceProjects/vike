        ...
        if (demographicsCSVData === null) {
            ...
            for (let row = 1; row < demographicsCSVData.length; row++) {
                ...
                if (demographicsCSVData[row].length > 1) { // avoid final blank row (if any)
                    let person = {ID: demographicsCSVData[row][0], LastName: demographicsCSVData[row][2], Pref_FirstName: demographicsCSVData[row][3], PersonType: demographicsCSVData[row][4],
                        Class: demographicsCSVData[row][5], Transfer: demographicsCSVData[row][6], ResidenceHall: demographicsCSVData[row][7], Gender: demographicsCSVData[row][8], 
                        Race: demographicsCSVData[row][9], Race_Desc: demographicsCSVData[row][10], International: demographicsCSVData[row][11]}
                    ...
                    firebase.database.ref('/demographics').child(email).set(person);
                }
            }
            this.setState({demographicsFile: null});
            alert("Please leave this window open for 5 minutes to allow the file to finish uploading.")
        }
    }
